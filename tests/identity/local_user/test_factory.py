...
def test_manufacture__with_valid_mapper_name(tweak_local_user_settings, mocker):
    mocked_ldap_instance = mocker.MagicMock()
    mocked_ldap_class = mocker.MagicMock(return_value=mocked_ldap_instance)
    mocker.patch.dict(
        "cluster_agent.identity.local_user.factory.mapper_map",
        {MapperType.LDAP: mocked_ldap_class},
    )
    with tweak_local_user_settings(LOCAL_USER_MAPPER=MapperType.LDAP):
        manufacture()
        mocked_ldap_instance.configure.assert_called_once_with(LOCAL_USER_SETTINGS)


def test_manufacture__with_invalid_mapper_name(tweak_local_user_settings, mocker):
    mocker.patch.dict(
        "cluster_agent.identity.local_user.factory.mapper_map",
        {"REAL": mocker.MagicMock()},
    )
    with tweak_local_user_settings(LOCAL_USER_MAPPER="FAKE"):
